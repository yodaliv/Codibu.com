...
@section('page-header')
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">{{__('Theme directory')}}</h1>
            <ol class="breadcrumb">
        ...
        <div class="col-12">
                ...
                <fieldset>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            ...
                            {{ Form::button('<i class="fa fa-search"></i> Search', ['type' => 'submit', 'class' => 'btn btn-primary btn-filter btn-block'])}}
                        </div>
                    </div>
                    <!--</form>-->
                </fieldset>
            ...
            <div class="themes row">
                @forelse($lists as $list)
                    <div id="theme-{{$list->id}}" class="theme-box col-md-4">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">{{$list->name}}</h5>
                                <small class="text-muted cat">
                                    <i class="icon icon-user text-info"></i> &nbsp;
                                    <a href="{{$list->developer_link}}" class="developer">{{$list->developer}}</a>
                                    &nbsp;
                                    <i class="icon icon-question text-info"></i> &nbsp;
                                    <a href="{{$list->info}}">Theme Info </a>
                                </small>
                                <p class="card-text truncate" style="min-height: 103px;">
                                    {!! $list->description !!}
                                    @if(strlen($list->description)>130)
                                        <span class="toggle-text-content">
                                        <a href="javascript:" class="toggle-text-link helpicon"
                                           onclick="readMoreLess(`theme-{{$list->id}}`)">Read more</a>
                                    </span>
                                    @endif
                                </p>
{{--                                <a href="{{$list->s3_url}}" download class="btn btn-dark download-theme mr-1 w-100">--}}
{{--                                    {{ __('Download Theme') }}--}}
{{--                                </a>--}}
                ...
                @empty
                    <div class="theme-box col-md-12">
                        <h4 class="text-center card p-3 font-weight-bold text-danger">No Themes found!</h4>
                    </div>
                @endforelse
            </div>
            <div class="theme-pagination mt-2 mb-5">
                {{$lists->links()}}
            </div>
        </div>
    </div>
...
@section('js')
    <script>
        ...
        function getLists(page) {
            ...
            window.location.href = `/directory/themes/list?${params}`;
        }
    </script>
@endsection
