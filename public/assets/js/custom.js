(function ($) {
	...
	$('.nav-link-header').on('click', function (e) {
		let notificationCount = $(this).attr('data-notification');
		if (notificationCount > 0) {
			$.ajax({
				...
				success: function success(response) {
					$('.nav-unread').css('visibility', 'hidden');
					$('.nav-link-header').attr('data-notification', 0);
				},
	...
	$(document).on('click', '.clear-all-notifi', function (e) {
		let notificationCount = $('.notifications-div').length;
		if (notificationCount > 0) {
			$.ajax({
				type: "get",
